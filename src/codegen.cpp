...
void double_funcs(FILE* fp) {
	...
	/* tgamma */
	{
		...
		constexpr int M = 45;
		double co[N][M];
		double polytable[] = { 0.422784335098467139, 0.644934066848226436, -0.202056903159594285, 0.0823232337111381915, -0.0369277551433699263,
				0.0173430619844491397, -0.00834927738192282684, 0.00407735619794433938, -0.00200839282608221442, 0.000994575127818085337, -0.000494188604119464559,
				0.000246086553308048299, -0.000122713347578489147, 0.0000612481350587048293, -0.0000305882363070204936, 0.0000152822594086518717,
				-7.63719763789976227e-6, 3.81729326499983986e-6, -1.90821271655393893e-6, 9.53962033872796113e-7, -4.76932986787806463e-7, 2.3845050272773299e-7,
				-1.19219925965311073e-7, 5.96081890512594796e-8, -2.98035035146522802e-8, 1.49015548283650412e-8, -7.45071178983542949e-9, 3.72533402478845705e-9,
				-1.86265972351304901e-9, 9.31327432419668183e-10, -4.65662906503378407e-10, 2.32831183367650549e-10, -1.16415501727005198e-10,
				5.82077208790270089e-11, -2.91038504449709969e-11, 1.45519218910419842e-11, -7.27595983505748101e-12, 3.63797954737865119e-12,
				-1.81898965030706595e-12, 9.09494784026388928e-13, -4.54747378304215403e-13, 2.27373684582465252e-13, -1.13686840768022785e-13,
				5.68434198762758561e-14, -2.84217097688930186e-14, 1.42108548280316068e-14, -7.10542739521085271e-15, 3.55271369133711367e-15,
				-1.77635684357912033e-15 };
		for (int n = 0; n < N; n++) {
			co[n][0] = lgamma(n + 1);
			double sum = -0.57721566490153286060651209008240243104215933593992L;
			for (int k = 1; k < n + 1; k++) {
				sum = sum + double(double(1.0) / double(k));
			}
			co[n][1] = sum;
			if (n == 1) {
				for (int m = 2; m < M; m++) {
					co[n][m] = polytable[m - 1] / m;
				}
			} else if (n == 0) {
				for (int m = 2; m < M; m++) {
					co[n][m] = polytable[m - 1] / m + pow(-1, m) / m;
				}
			}
		}
		fprintf(fp, "\n");
		...
		fprintf(fp, "\t};\n");
		fprintf(fp, "\tsimd_f64 y, s, x0, x1, xi, sgn;\n");
		fprintf(fp, "\tsimd_i64 i, nn;\n");
		fprintf(fp, "\tsimd_f64_2 Z, Y, X;\n");
		fprintf(fp, "\tint j;\n");
		fprintf(fp, "\ts = copysign(simd_f64(1), x);\n");
		fprintf(fp, "\tx0 = floor(x);\n");
		fprintf(fp, "\tx1 = x - x0;\n");
		//fprintf(fp, "\tn = simd_i64(x0) - simd_i64(1);\n");
		fprintf(fp, "\ti = x1 > simd_f64(0.5);\n");
		fprintf(fp, "\tx1 = blend(x1, x1 - simd_f64(1), i);\n");
		fprintf(fp, "\ti = -i;\n");
		fprintf(fp, "\tj =  _mm256_movemask_pd(((simd_f64&) i).v);\n");
		fprintf(fp, "\tX = x1;\n");
		fprintf(fp, "\tY = co[j][%i];\n", (M - 1));
		for (int n = M - 2; n >= 0; n--) {
			fprintf(fp, "\tY = (X * Y +  co[j][%i]);\n", n);
		}
		fprintf(fp, "\ty = exp(Y.x)*(1.0+Y.y);\n");
		fprintf(fp, "\txi = -floor(x - simd_f64(1));\n");
		fprintf(fp, "\tnn = floor(fabs(x - simd_f64(1))) + (x < simd_f64(1));\n");
		fprintf(fp, "\tsgn = copysign(1.0, -xi);\n");
		fprintf(fp, "\tZ = simd_f64_2(simd_f64(1));\n");
		fprintf(fp, "\txi += (sgn - simd_f64(1)) * simd_f64(0.5);\n");
		for (int n = 0; n < 160; n++) {
			fprintf(fp, "\tZ = blend(Z, Z * simd_f64_2::quick_two_sum(xi, x), nn > simd_i64(0));\n");
			fprintf(fp, "\txi += sgn;\n");
			fprintf(fp, "\tnn -= simd_i64(1);\n");
		}
		fprintf(fp, "\tY = y;\n");
		fprintf(fp, "\ty = blend((Y * Z).x, (Y / Z).x, sgn < simd_f64(0));\n");
		fprintf(fp, "\treturn y;\n");
		fprintf(fp, "}\n");
		...
		/*
		 ...
		 xi += (sgn - 1) / 2;
		 while (nn > 0) {
		 Z = Z * double_2::quick_two_sum(xi, x);
		 xi += sgn;
		 nn--;
		 }
		 if (sgn < 0) {
